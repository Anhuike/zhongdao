<template>
	<view class="">
		...
		<view class="video_box">
			<view class="video">
				<video id="videoZiXun" :src="detail.video_url" controls><!-- autoplay -->
					<!-- #ifdef H5 -->
					<cover-image @click="back" v-if="isIOS == 0 && is_weixin == 0" style="margin-top: 6upx;" class="btn_back" src="../../static/postdetails/btn_nav_backblack@2x.png"></cover-image>
					<!-- #endif -->
					<!-- #ifndef H5 -->
					<cover-image @click="back" style="margin-top: 50upx;" class="btn_back" src="../../static/postdetails/btn_nav_backblack@2x.png"></cover-image>
					<!-- #endif -->
					<!-- #ifndef MP -->
					<!-- <cover-image class="btn_zx_share" @click="moreShare(detail.thumb,detail.title,detail.link)" src="../../../static/postdetails/btn_nav_shareblack@2x.png"></cover-image> -->
					<!-- #endif -->
				</video>
			</view>
			<!-- <image @click="back" style="top: 6upx;" class="btn_back" src="../../static/postdetails/btn_nav_backblack@2x.png" mode=""></image> -->
		</view>
		<!-- 顶层视频 end -->
		
		<!-- 顶部 end -->
		<!-- <view>
			<states :top="'423rpx'" :marT="'0'" :Fix='true' :label="1" :reason="2" @statesHeight="statesHeight"></states>
		</view> -->
		
		<view>
			<scroll-view class="videoScroll" scroll-y scroll-with-animation="true" :scroll-into-view='toView' @scroll="turnScroll"  @scrolltolower="scrolltoGetlowerComment" >
				<!-- 状态栏距离 -->
				...
				<view class="z_v_content" v-html="content"></view>
		...
		<view class="bottom" :style="judiflag ? 'height: 116rpx;' : 'height: 90rpx;'">
			<view class="bottom_box">
				...
				<view class="bottom_icon" @click="returnComment">
					<image class="bottom_img" :src="btn_bottom_like" mode=""></image>
		...
		<view>
			...
			<view class="replayV" :style="replayFn ? 'transform: translateY(0%);-webkit-transform: translateY(0%);' : 'transform: translateY(100%);-webkit-transform: translateY(100%);'">
				...
				<view>
					<scroll-view class="replay_scroll" scroll-y @scrolltolower="scrolltoGetlower">
						...
						<view class="ti_con" @click="replayLz">
							<view v-html="replayComments.content"></view>
						</view>
				...
				<view class="replay_input" :style="judiflag ? 'height: 116rpx;' : 'height: 90rpx;'">
					<!-- #ifndef APP-PLUS -->
					<input class="replay_input_in"  @input='onKeyReplyWord' @confirm="onKeyReply" v-model="replayContent" :focus="replayFocus" confirm-type="send" type="text"  :placeholder="replayPlace" placeholder-style="font-size: 28upx; color: #999999; "/> <!-- v-model="keyReplay" -->
					<!-- #endif -->
					<!-- #ifdef APP-PLUS -->
		...
		<userSare :shaflag="shaflag" :shareTitle="detail.title" :shareContent="detail.content" :shareLink="detail.link" :shareThumb="detail.thumb" :isShowBottom="isShowBottom" @closeShaflag="closeShaflag"></userSare>
	...
	export default {
		...
		data() {
			return {
				...
				btn_bottom_like:require("../../static/mine_img/btn_bottom_like@2x.png"),
			}
		},
		// onLoad(options) {
		...
		mounted() {
			...
			this.content = this.detail.content.replace(/\n/g,"<br/>")
...
<style scoped>
	.video_box{width: 100%;height: 423upx;position: relative;z-index: 4;background: rgba(0,0,0,0.7);}
	.btn_back{width: 34px;height: 34px;margin-left: 6upx;z-index: 2;}
	.btn_zx_share{width: 34px;height: 34px;position: absolute;right: 6upx;top: 6upx;margin-top: var(--status-bar-height);}
	.video{position: fixed;z-index: 4;top: 0;left: 0;right: 0;height: 423upx;z-index: 2;}
	.video video{width: 100%;height: 423upx;position: relative; z-index: 4;}
	
	.videoScroll{position: absolute;top: 423upx;left: 0;right: 0;bottom: 0;z-index: 1;background: #F6F6F6;}
	.zixun_title{font-size: 36upx;color: #333333;font-weight: bold;padding: 24upx;background: #FFFFFF;}
	.z_v_content{font-size: 28upx;color: #333333;padding: 0 24upx 24upx;background: #FFFFFF;}
	...
	.chat_box{position: fixed;top: 423upx;left: 0;right: 0;bottom: 0;background: #FAFAFA;z-index: 100;transition: all linear .3s;transform: translateY(100%);}
